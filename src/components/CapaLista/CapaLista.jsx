...
const [capasFiltradas, setCapasFiltradas] = useState([]);
  ...
  const filtroPorTitulo = ({target}) => {
    const lista = [...capas].filter(({titulo}) => matchByText(titulo, target.value))
  ...
  useEffect(() =>{getLista()},[capaEditada, capaRemovida]);
  ...
  useEffect(() => {
    if (
      capaCriada &&
      !capas.map(({ id }) => id).includes(capaCriada.id)
    ) {
  ...
  capasFiltradas.map((capa, index) => (
    ...
    />
      ...
      {capaModal && (<CapaDetalhesModal capa = {capaModal} closeModal = {() => setCapaModal (false)} />)};
    </div>
    </div>
  );
};
